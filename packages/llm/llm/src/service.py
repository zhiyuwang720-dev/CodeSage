...
一个组合里要跑模型,它该面向什么编程?答案不是某一家提供者的
SDK,而是本包定义的服务契约:请求消息用 ContentBlock(仓库内部
消息契约,全系统唯一形状),调用经 complete/stream 发起,配置经
指针解析。消费者只认识这份契约。

提供者怎么进来?经能力接缝:register_provider 把「提供者名 →
适配器工厂」注册到服务上,底层是包内的 LLMClient(重试、回退、
成本、流式组装都是它的本职)。注册挂在服务所在 fiber 的生命周期
上 —— 组合卸载,注册自动撤销,不留幽灵适配器。
