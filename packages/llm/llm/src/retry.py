...
def is_ptl_text(text: str) -> bool:
    """Text-level PTL hint: OpenAI-compatible `context_length_exceeded`,
    Anthropic `prompt_too_long`, or a generic context-length mention."""
    msg = (text or "").lower()
    return any(
...
def is_ptl_error(exc: LLMError) -> bool:
    ...
    error code. with_retry never retries these (non-retryable 4xx); the
    loop's reactive-compaction path owns them (specs/08 §3.8)."""
    if exc.cancelled:
        return False
