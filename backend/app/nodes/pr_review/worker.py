...
async def _node_heartbeat(ctx: dict[str, Any]) -> None:
    from app.control_plane.scale_ops.node_registry import agent_node_registry
    from app.db.session import async_session_factory

    identity = ctx["node_identity"]
    while True:
        await asyncio.sleep(settings.PR_NODE_HEARTBEAT_SECONDS)
        async with async_session_factory() as db:
            if not await agent_node_registry.heartbeat(db, identity):
                raise RuntimeError("agent node instance was revoked")
...
async def execute_agent_task_job(
    ...
    try:
        with get_tracer().start_as_current_span("worker.receive", context=parent) as receive_span:
            ...
            executor = ctx.get("execute_agent_task")
...
async def startup_pr_node(ctx: dict[str, Any]) -> None:
    from app.control_plane.scale_ops.node_registry import agent_node_registry
    from app.db.session import async_session_factory

    await startup_observability(ctx)
    capabilities = tuple(
        item.strip() for item in settings.PR_NODE_CAPABILITIES.split(",") if item.strip()
    )
    async with async_session_factory() as db:
        identity = await agent_node_registry.register(
            db,
            node_id=settings.PR_NODE_ID,
            agent_type="pr_review",
            entrypoint="quick_review",
            agent_version=settings.PR_NODE_AGENT_VERSION,
            protocol_version=settings.PR_NODE_PROTOCOL_VERSION,
            capabilities=capabilities,
            max_concurrency=settings.AGENT_WORKER_CONCURRENCY,
        )
    ctx["node_identity"] = identity
    ctx["node_heartbeat_task"] = asyncio.create_task(_node_heartbeat(ctx))
    logger.info(
        "PR node registered node_id=%s instance_id=%s pid=%s",
        identity.node_id,
...
async def shutdown_pr_node(ctx: dict[str, Any]) -> None:
    from app.control_plane.scale_ops.node_registry import agent_node_registry
    from app.db.session import async_session_factory

    heartbeat = ctx.get("node_heartbeat_task")
    ...
    await shutdown_observability(ctx)
...
class WorkerSettings:
    ...
    on_startup = startup_pr_node
    on_shutdown = shutdown_pr_node
