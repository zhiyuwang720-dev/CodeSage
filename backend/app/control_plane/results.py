...
class ReviewResultService:
    ...
    async def touch_session(self, db, task_id: str, perspective: str, session_id: str) -> None:
        await self.start_perspective(db, task_id, perspective, session_id=session_id)

    async def start_perspective(
        self, db, task_id: str, perspective: str, *, session_id: str
    ) -> None:
        await audit_stage_store.start(
            db, task_id, f"review:{perspective}", session_id=session_id
        )
