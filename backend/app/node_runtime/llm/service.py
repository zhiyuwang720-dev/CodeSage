...
class LLMService:
    ...
    def __init__(
        ...
        *,
        analysis_policy_loader: Callable[..., Awaitable[tuple[str | None, list | None]]]
        | None = None,
    ):
        self._user_config = user_config or {}
        self._configs: Dict[Optional[str], LLMConfig] = {}
        self._analysis_policy_loader = analysis_policy_loader
    ...
    async def analyze_code_with_rules(
        ...
        if db_session is not None and self._analysis_policy_loader is not None:
            try:
                actual_language = output_language or self._get_output_language()
                custom_prompt, rules = await self._analysis_policy_loader(
                    db_session,
                    rule_set_id=rule_set_id,
                    prompt_template_id=prompt_template_id,
                    use_default_template=use_default_template,
                    output_language=actual_language,
                )
            except Exception:
