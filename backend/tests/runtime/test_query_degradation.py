...
from app.contracts.models import RuntimeContinueReason, RuntimeMessageRole, RuntimeModelResponse, RuntimeStopReason, TranscriptItem
...
def test_reactive_compact_retry_uses_partial_compaction_service(monkeypatch):
    ...
    async def fake_partial_compact_conversation(all_messages, pivot_index, state, model, model_client=None, user_feedback=None, direction="from", **kwargs):
        ...
        return __import__("app.node_runtime.harness.compaction.models", fromlist=["CompactionResult"]).CompactionResult(
    ...
    monkeypatch.setattr(
        "app.node_runtime.harness.query_degradation.partial_compact_conversation",
