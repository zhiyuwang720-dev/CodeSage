...
class OpenAICompatibleAdapter(BaseAdapter):
    ...
    def _message_to_openai(self, msg: Message) -> list[dict[str, Any]]:
        ...
        blocks = msg.content
        # tool_result blocks become role=tool messages; everything else is
        # assembled into a single content list per message.
    ...
    async def astream(self, request: LLMRequest) -> AsyncIterator[StreamEvent]:
        ...
        async with self.http.stream(
            ...
            if response.status_code >= 400:
                yield StreamEvent(type="error", error=f"HTTP {response.status_code}: {response.status_code}")
                return
...
def _usage_from_openai(usage: dict[str, Any] | None) -> Usage | None:
    ...
    hit = usage.get("prompt_cache_hit_tokens") or usage.get("prompt_tokens_details", {}).get("cached_tokens", 0)
    miss = usage.get("prompt_cache_miss_tokens") or usage.get("prompt_tokens", 0) - hit
    return Usage(
        input_tokens=miss,
