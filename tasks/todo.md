...
直接回答:约 2.0–2.2 万行 Python(代码 ~1.2–1.3 万 + 测试 ~8 千),分 3–4 批

映射系数(实测,不是拍脑袋)

我们已实现的 llm 家族 + scope = 3,596 行 Python,对应 DSH 的 TS ≈ 5,700–5,900 行(llm 2,771 + deepseek 1,491 + token-meter 999 + scope) → 系数 ≈ 0.6。TS 的 zod 校验、声明合并、类型样板在 Python 里大量消失。

最小可跑 Harness 的构成(照 DSH core 结构)

┌─────────────────────────────────────────────────────────┬─────────────────────┬───────────────────────────┐
│                          组件                           │     DSH TS 行数     │        Python 预估        │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ core/session 三包(本次)                                 │ 6,889(砍 zstd 374)  │ ~4,000 代码 + ~3,000 测试 │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ core/agent + agent-loop                                 │ 3,298               │ ~2,000 + ~1,500           │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ system-prompt + agent-default-model + tool-presentation │ ~1,200              │ ~800                      │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ core/tools(只做核心子集:fs/bash/glob/grep)              │ 5,628 → 子集 ~2,000 │ ~1,200 + ~1,000           │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ llm 家族                                                │ 5,261               │ ✅  已实现 ~2,000          │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ settings + context + todo + hooks                       │ ~4,500              │ ~2,500 + ~1,500           │
├─────────────────────────────────────────────────────────┼─────────────────────┼───────────────────────────┤
│ compaction + preset(可后置)                             │ ~2,500              │ ~1,500                    │
└─────────────────────────────────────────────────────────┴─────────────────────┴───────────────────────────┘

本次三包明细(已批准计划)

7,263 行 TS(src 精确统计)→ Python ~4,000 行代码 + ~3,000 行测试 ≈ 7,000 行,一次会话可完成。

两个参照系

- 我们现有 codesage:22,662 代码 + 22,819 测试 ≈ 4.5 万行(19 阶段老架构)。DSH 路线的最小 harness 反而比老架构小一半 —— 插件化 + 事件溯源省掉了大量状态管理代码,这是架构红利。
- DSH 全仓库:88.6 万行 TS(49 顶层包)是完整产品(客户端 UI、web、LSP、sandbox、subagent 全家族),不在考虑范围。

批次节奏

1. 本次:session 三包(~7,000 行)
2. agent 循环 + tools 核心(~6,500)
3. settings/context/todo/hooks/compaction(~5,500)
4. 外围(query/title/telemetry/sqlite)按需再加

结论:照 DSH 路线重建整个 harness ≈ 2 万行 Python,3–4 批完成,总规模只有旧 codesage 的一半。继续按批准的计划开工?