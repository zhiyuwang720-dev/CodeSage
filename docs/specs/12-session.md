...
2. **信息密度分层**(对齐 14.11「成本精度分档」思想):概览少信息、细节全信息 —— `/sessions` 一行一会话,id 只显前 4 位十六进制(`3f9a…`);`/tree` 每行截断 80 字符;全量内容走 `/tree <entryId>` 上下文窗口。**截断是显示层行为,数据层全量保留**(append-only)。
...
**渲染示例**(信息密度分层 §1.4:id 前 4 位十六进制 + 一行一会话 + 无标题 `(untitled)`):
